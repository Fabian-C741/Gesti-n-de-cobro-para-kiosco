...
function abrirEscanerProducto(campoId) {
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body p-0">
                        ...
                        <div id="scannerStatusProd" class="p-3 text-center"></div>
                    </div>
                    <div class="modal-footer">
    ...
    document.getElementById('scannerStatusProd').innerHTML = '<div class="spinner-border text-primary" role="status"></div><p class="mt-2 mb-0">Cargando...</p>';
    ...
    if (navigator.permissions && navigator.permissions.query) {
        navigator.permissions.query({ name: 'camera' }).then(result => {
            ...
            if (result.state === 'denied') {
                mostrarInstruccionesDesbloqueoProd();
            } else {
                intentarAccesoCamaraProd();
            }
        }).catch(() => intentarAccesoCamaraProd());
    } else {
        intentarAccesoCamaraProd();
    }
}

function mostrarInstruccionesDesbloqueoProd() {
    const statusDiv = document.getElementById('scannerStatusProd');
    statusDiv.innerHTML = `
...
function intentarAccesoCamaraProd() {
    const statusDiv = document.getElementById('scannerStatusProd');
    ...
    navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment" } })
        ...
        .catch(err => {
            ...
            if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
                ...
                statusDiv.innerHTML = `<div class="alert alert-warning mb-3"><i class="bi bi-exclamation-triangle me-2"></i><strong>Cámara ocupada</strong><br><small>Cierra otras apps que usen la cámara</small></div><button class="btn btn-primary" onclick="solicitarPermisoCamaraProd()"><i class="bi bi-arrow-clockwise me-2"></i>Reintentar</button>`;
            } else {
                statusDiv.innerHTML = `<div class="alert alert-danger mb-3"><strong>Error:</strong> ${err.message || err.name}</div><button class="btn btn-primary" onclick="solicitarPermisoCamaraProd()"><i class="bi bi-arrow-clockwise me-2"></i>Reintentar</button>`;
            }
        });
}
